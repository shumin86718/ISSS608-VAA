...
MC2_edges_aggregated <- MC2_edges_clean_mapped %>%
  ...
  filter(fishtype %in% c("crustaceans")) %>%
...
quantile_graph <- quantile(eigen_centrality(MC2_graph_r)$vector,
         probs = seq(0, 1, 1/10)
         )
...
g <- MC2_graph_2 %>%
  ...
  geom_edge_link(aes(width = Weight), alpha = 0.2) +
  scale_edge_width(range = c(0.1, 5)) +
  geom_node_point(aes(color = community), show.legend = FALSE) +
  geom_node_text(aes(label = ifelse(betweenness_centrality > quantile(betweenness_centrality, 0.8), id, "")), 
